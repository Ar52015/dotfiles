...
6. Generate the schedule following the structure and guidelines below. **Cap output at 10 days per generation.** If the schedule requires more than 10 days, stop after Day 9 (or the 10th day, counting Day 0) and append a continuation block (see OUTPUT STRUCTURE). The continuation block is a self-contained prompt the user can paste back into `/roadmap` to generate the next batch — it must carry all context needed so the next invocation can proceed without re-asking any questions. Each continuation batch picks up where the last left off.
   ...
   **Before finalizing the draft**, fill out the Design Decisions block (see OUTPUT STRUCTURE) — every field requires a project-specific justification and a rejected alternative. If any field reads like template boilerplate, you haven't thought about it hard enough. User-pinned technologies still need justification; if none emerges, flag it as a reconsideration signal. Then check for these design smells and fix in place:
   - Any day whose strongest argument for existing is "the template says so" → find a project-specific reason or cut/restructure.
   - Any day that's purely research with no deliverable → fold into a foundation day or the day it serves.
   - Back-to-back days with no shared dependency → note they can be parallelized.
   - Risk concentrated at the end → reorder to confront the riskiest unknown earlier.
   - If 30% of days were cut, which go? If a day survives only by ceremony, it's bloat.

7. **Verification pass.** After generating the full schedule, re-read it and verify every specific claim against the docs fetched in step 5:
   ...
   - Any section names referenced in resource annotations (e.g., "Focus on **Section X**") — does that section actually appear on the linked page? WebFetch the URL and confirm before including the annotation.
...
**Walking skeleton**: <the minimal runnable end-to-end slice Day 1 produces — one sentence. Minimum bar: for backend/systems, a request enters → logic runs → response or side effect comes out. For frontend, a route renders → fetches from stub API → displays data with loading/error states → passes axe-core. Anything less and Day 2 is wiring the skeleton instead of adding features.>
...
1. **Tooling & Environment** — Package manager init, dependency installation, linters, type checkers, formatters, pre-commit hooks, `.editorconfig`, project skeleton. Always Day 0. Adapt to the project's language stack (e.g. Python: `uv init`/`uv add`/`uv sync`, ruff, mypy; Go: `go mod init`, golangci-lint; Rust: `cargo init`, clippy; Node: `npm init`, eslint, tsc; C/C++: CMake, `.clang-format`, `.clang-tidy`).
...
3. **Contracts & Interfaces** — Protobuf schemas, API specs, DB schemas, type definitions. For frontend projects: design tokens, component prop contracts, route definitions, shared UI primitives. For product projects with persistence, include migration tooling, schema versioning, backup and recovery strategy, and API contract testing if multiple consumers exist. Feature flags for gradual rollout where applicable. Shape of data before code.
4. **Core Implementation** — Servers, clients, business logic, handlers. For frontend projects: components, hooks, stores, routers, feature slices. Error handling at boundaries is part of implementation, not an afterthought — validate inputs (with a security lens: injection, path traversal, OWASP top 10 where applicable), type errors, and define propagation strategy alongside the code that produces them. For frontend work, **accessibility and responsive design are continuous concerns, not polish items**: every component-building day has an a11y acceptance criterion (semantic HTML, keyboard navigation, axe-clean, focus management), and breakpoint systems are decided alongside component APIs on Day 0, never retrofitted.

   **Decomposition strategy** — choose based on the project's architecture:

   | Strategy | Default for | Each day delivers |
   |---|---|---|
   | Vertical slicing | multi-feature products | one thin end-to-end feature |
   | Inside-out | pipeline/library projects | next layer of core logic, then its boundary adapter |
   | Outside-in | spec/protocol-driven projects | entry point first, stub internals, fill in |
   ...
   **Testing**: tests are part of each implementation day, never a separate phase. Every day that writes code includes tests for that code in the same day. To decide test level, pick from this taxonomy:
   ...
   Each day names which levels its code warrants. For learning projects, add a `Note:` explaining *why* each test exists and what it catches. Measure coverage if useful, but never gate CI on it — coverage targets incentivize testing trivial code.

5. **Integration & Infrastructure** — Dockerfiles, Compose, networking, volumes, wiring. For frontend projects: bundler pipeline (Vite/Webpack/Turbopack), CDN deployment, preview environments, edge config, asset optimization. End-to-end flows, startup ordering. For product projects with services, include health check endpoints, graceful shutdown, rate limiting, and circuit breakers. For product projects deploying to cloud, add Infrastructure as Code (Terraform/Pulumi).
6. **Performance & Validation** — Rate control, benchmarks, soak tests, profiling. For frontend projects: Core Web Vitals (LCP/INP/CLS), Lighthouse audits, bundle size budgets, accessibility audits, visual regression. Only after baseline works end-to-end. Include smoke tests that verify the deployed/built artifact actually works (not just CI green). For product projects with traffic, add observability (metrics via Prometheus/OpenTelemetry, tracing) — structured logging is the entry point, full observability when there's something to measure. For product projects with deployed services, add error monitoring and alerting (Sentry, Datadog, or equivalent), SLOs/error budgets, and CORS/CSP/security headers for web-facing services.
7. **DevOps & Delivery** — Always included, both project types. CI/CD pipelines, Makefile or task runner, multi-stage Dockerfile for reproducible build-and-test from a clean machine. Minimum: CI with lint + build + test on every push/PR, a Makefile wrapping all project commands (build, test, lint, format, bench, validate, docker, and a `clean` target that removes all project-generated artifacts — build dirs, caches, venvs, downloaded deps, compiled outputs, container images, pre-commit envs), a multi-stage Dockerfile (build stage + slim runtime stage), license compliance check on dependencies, and a documented rollback strategy (even if just "revert the commit and redeploy"). For product projects, also include: SAST on own code (Semgrep, CodeQL, or Bandit — not just dependency scanning), secrets management beyond env vars (rotation, auditing), security scanning in CI (dependency scanning + secrets scanning, using ecosystem-appropriate tools — Dependabot/Snyk for pip/npm/cargo/go, skip for C/C++ with vendored or FetchContent deps), release engineering with artifact publishing (semver for libraries, calver for applications, changelogs from conventional commits, git tags, publish to PyPI/npm/crates.io/Docker Hub), and dependency update automation (Dependabot/Renovate where the ecosystem supports it).
8. **Documentation & Polish** — Always present as the final day in the schedule, marked optional for the developer to skip. README (project description, prerequisites, quick start, available make targets). For product projects, also include: SECURITY.md (how to report vulnerabilities), ADRs for architecture decisions, auto-generated API docs, architecture diagrams.
...
- **Day 0:** Always Tooling & Environment + Config/Utilities. Pure setup — no business logic. Has Focus, Load, Tasks, and Resources — no Objectives or Acceptance Criteria. Resources should cover the tools being configured (package manager, build system, linters, pre-commit). Must include: language-appropriate package manager init and dependency installation, pre-commit hooks (`.pre-commit-config.yaml` with `trailing-whitespace`, `end-of-file-fixer`, `check-yaml`, `check-toml`, plus local hooks for the project's linter/formatter/type-checker, and a `commit-msg` hook for conventional commit enforcement via commitlint or equivalent), linter and formatter config, strict type checking where the language supports it. Read the user's `~/.claude/CLAUDE.md` for preferred tooling (e.g. package managers, linters).
- **Foundation Days:** See step 4 for trigger rules and scoping. Use the foundation day template (see OUTPUT STRUCTURE). One rule the template can't express: **domain foundation day exercises use a language the user already owns at Tier 1** (confront domain invariants without library friction); **technology foundation day exercises use the project's target language and library.**
- **Load Levels:** L1: Config/boilerplate (<2h). L2: Foundational setup (2-4h). L3: Domain knowledge required (4-6h). L4: Multi-component integration (6-8h). L5: Performance-critical or architecturally complex (8+h). Always follow the level with a one-line reason tied to the actual work shape.
- **Preflight Checklist:** If the checklist would be empty (fully self-contained project), omit the section entirely.
- **If Stuck:** 2–5 project-specific failure modes and first-try remediations per day. Not generic ("check the logs") — specific to that day's tasks. For foundation days, scope to Exercise 1/2 failure modes.
- **Resources:** Official docs first, other sources as supplements or easier entry points — never instead of official docs. For each link, specify which sections to read, what to skip, and what the reader will learn. List in reading order. Gate total reading volume to keep each day feasible, but never remove a resource necessary for that day's tasks. Adjust style by tier: Tier 1 reference-only, Tier 2 refresher + references, Tier 3 tutorial-style + references, Tiers 4-5 get resources on foundation days. For tasks that produce a configuration artifact (build file, CI config, Dockerfile), include at least one resource showing a complete working example — reference docs explain individual pieces but don't teach composition.
- **Resource fallbacks:** When a primary resource is too dense or advanced for the user's tier, attach: `*If the above is unclear:* [Fallback](<URL>) — <when this is the better entry point>`. Not every resource needs one — only when the primary is likely to block the reader.
